...
When we manipulate identifiers, we use the following definitions:
...
*Side note: as shown above, `asCql` has a pretty-printing option that omits the quotes if possible.
This looks nicer, but is slightly more expensive because it requires parsing the string.*
...
The Java driver's team recommendation is:
